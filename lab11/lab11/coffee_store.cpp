#include "coffee_store.hpp"

void Customer::notify(std::string n){
    if(n == name_){
        std::cout << name_ << " grabbed their drink." << std::endl;
    }
}
...
void Barista::add_drink(Coffee c, Level l){
    if(l == lvl_ || super_ == nullptr){
        ...
        switch(lvl_){
            case JuniorBarista:
                std::cout << "a Junior Barista." << std::endl;
                break;
            case SeniorBarista:
                std::cout << "a Senior Barista." << std::endl;
                break;
            case Manager:
                std::cout << "a Manager." << std::endl;
                break;
        }
        orders_.push_back(c);
    }else{
...
void Barista::print_order_complete(Coffee &c){
    ...
    std::cout << " with ";
    c.print_toppings();
    std::cout << " is ready. It will cost $" << c.getPrice() << ", please." << std::endl;
}
...
Coffee Coffee_Store::buy_drink(){
    Coffee myCoffee;
    myCoffee.order_coffee();
    return myCoffee;
}

void Coffee_Store::run_store(){
    char repeat;
    do{
